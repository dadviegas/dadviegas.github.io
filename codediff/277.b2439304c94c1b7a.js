"use strict";(self.webpackChunk_atlantis_codediff=self.webpackChunk_atlantis_codediff||[]).push([["277"],{1315(e,o,a){function t(e,o){let a=e.split("\n"),t=o.split("\n");if(a.length>2e3||t.length>2e3){var r=a,i=t;let e=[],o=Math.max(r.length,i.length);for(let a=0;a<o;a++)a>=r.length?e.push({type:"insert",oldLine:null,newLine:i[a],oldNum:null,newNum:a+1}):a>=i.length?e.push({type:"delete",oldLine:r[a],newLine:null,oldNum:a+1,newNum:null}):r[a]===i[a]?e.push({type:"equal",oldLine:r[a],newLine:i[a],oldNum:a+1,newNum:a+1}):e.push({type:"replace",oldLine:r[a],newLine:i[a],oldNum:a+1,newNum:a+1});return e}let n=function e(o,a,t,r,i){return 0===r&&0===i?[]:r>0&&i>0&&a[r-1]===t[i-1]?[...e(o,a,t,r-1,i-1),{type:"equal",oldLine:a[r-1],newLine:t[i-1],oldNum:r,newNum:i}]:i>0&&(0===r||o[r][i-1]>=o[r-1][i])?[...e(o,a,t,r,i-1),{type:"insert",oldLine:null,newLine:t[i-1],oldNum:null,newNum:i}]:[...e(o,a,t,r-1,i),{type:"delete",oldLine:a[r-1],newLine:null,oldNum:r,newNum:null}]}(function(e,o){let a=e.length,t=o.length,r=Array.from({length:a+1},()=>Array(t+1).fill(0));for(let i=1;i<=a;i++)for(let a=1;a<=t;a++)e[i-1]===o[a-1]?r[i][a]=r[i-1][a-1]+1:r[i][a]=Math.max(r[i-1][a],r[i][a-1]);return r}(a,t),a,t,a.length,t.length),l=[],s=0;for(;s<n.length;)"delete"===n[s].type&&s+1<n.length&&"insert"===n[s+1].type?(l.push({type:"replace",oldLine:n[s].oldLine,newLine:n[s+1].newLine,oldNum:n[s].oldNum,newNum:n[s+1].newNum}),s+=2):(l.push(n[s]),s++);return l}function r(e){let o=0,a=0,t=0;for(let r of e)"insert"===r.type?o++:"delete"===r.type?a++:"replace"===r.type&&t++;return{added:o,removed:a,changed:t}}a.d(o,{P:()=>t,h:()=>r})},5151(e,o,a){a.d(o,{k:()=>t});let t=[{label:"TypeScript function refactor",original:`function fetchUser(id) {
...
`,V.A,V.A['pre[class*="language-"]'],s.w4.colors.codeBg,V.A['code[class*="language-"]'],l().div`
  background: ${({bg:e})=>e};
...
`,l().aside`
  ...
  display: flex;
  flex-direction: column;
  animation: ${N} 0.22s cubic-bezier(0.4, 0, 0.2, 1) both;
